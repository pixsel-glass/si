...
final class Smsfeedback extends SmsGate {
    ...
    public function send() {
        ...
        if ($this->username && $this->password) {
            if ($this->from) {
                ...
                $sender = 'TEST';
                $sms_log->write('(Smsfeedback) Notice: Default Sender set! Please input real Sender');
            }

            $balance = $this->getBalance($this->username, $this->password);

            if ($balance[1]) {
                $sms_log->write('(Smsfeedback) : Balance: ' . $balance[1]);
                ...
                if ($this->to) {
                    ...
                    $sms_log->write('(Smsfeedback) : SmsSend: Response: ' . $response[0] . ' ID: ' . $response[1]);

                    $status = $this->getStatus($this->username, $this->password, $response[1]);

                    $sms_log->write('(Smsfeedback) : SmsSend: ID: ' . $status[0] . ' Status: ' . $status[1]);
                } else {
                    $sms_log->write('(Smsfeedback) Error: Phone destination not found!');
                }
                ...
                $sms_log->write('(Smsfeedback) : Current Balance is 0, Sms not send or Authorisation fault');
            }
        } else {
            $sms_log->write('(Smsfeedback) Error: Please set correct login/password!');
        }
    }
