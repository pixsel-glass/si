...
class SeoUrlGenerator
{
    public function translitDo($path, $string, $translit_function, $licence_code)
    {
        ...
        if (is_file($inc_file)) {
            ...
            $this->log->write("ERROR -- SEO URL Generator: function  " . $translit_function . "() is not calable" . " in " . basename(__FILE__) . " on line " . "23" . ". Code: if (is_callable(\$translit_function)) {");
        } else {
            $this->log->write("ERROR -- SEO URL Generator: No file  " . $inc_file . " required" . " in " . basename(__FILE__) . " on line " . "20" . ". Code: require_once \$inc_file;");
        }
    ...
    public function clearString($string, $setting, $licence_code)
    {
        ...
        return str_replace(["ʼ", "`", "~", "@", "#", "№", "\$", "%", "^", "&", "*", "(", ")", "+", "-", "=", ".", ":", ",", ";", "!", "?", "—", "'", "\"", "\\", "/", "{", "}", "[", "]", "<", ">", "°", "•", "″", "×", "÷", "ø", "²"], " ", $string);
    }
    public function isValidLicence($licence)
    {
        $a_licence = $this->decodeLicence($licence);
        // $licenced_site = str_replace(["http:", "https:", "//", "www."], ["", "", "", ""], mb_strtolower($a_licence["site"], "UTF-8"));
