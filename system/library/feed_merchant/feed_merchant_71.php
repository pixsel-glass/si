...
class ControllerExtensionFeedMerchantFeed extends Controller
{
    ...
    public function index()
    {
        ...
        $data["breadcrumbs"][] = ["text" => $this->language->get("text_extension"), "href" => $this->url->link("extension/extension", "user_token=" . $this->session->data["user_token"] . "&type=feed", true)];
        $data["breadcrumbs"][] = ["text" => $this->language->get("heading_title"), "href" => $this->url->link("extension/feed/merchant_feed", "user_token=" . $this->session->data["user_token"], true)];
        ...
        $data["feed_merchant_feed_settings"] = [];
        if ($merchant_feed_settings) {
            foreach ($merchant_feed_settings as $tab => $setting) {
                ...
                if (!empty($setting["product_stock_status"])) {
                    ...
                    $data["feed_merchant_feed_settings"][$tab]["product_stock_status"] = [];
                }
                ...
                if (!empty($setting["product_category"])) {
                    ...
                    $data["feed_merchant_feed_settings"][$tab]["product_category"] = [];
                }
                if (!empty($setting["manufacturer_category"])) {
                    ...
                    $data["feed_merchant_feed_settings"][$tab]["manufacturer_category"] = [];
                }
                ...
                $data["feed_merchant_feed_settings"][$tab]["product_black_list"] = [];
                if (!empty($setting["product_black_list"])) {
                    ...
                    foreach ($products as $product_id) {
                        ...
                        if ($related_info) {
                            $data["feed_merchant_feed_settings"][$tab]["product_black_list"][] = ["product_id" => $related_info["product_id"], "name" => $related_info["name"]];
                        }
                    }
                }
            }
    ...
    public function category()
    {
        ...
        $data["breadcrumbs"][] = ["text" => $this->language->get("text_extension"), "href" => $this->url->link("extension/extension", "user_token=" . $this->session->data["user_token"] . "&type=feed", true)];
        $data["breadcrumbs"][] = ["text" => $this->language->get("heading_title"), "href" => $this->url->link("extension/feed/merchant_feed", "user_token=" . $this->session->data["user_token"], true)];
        $data["breadcrumbs"][] = ["text" => $this->language->get("heading_title_category"), "href" => $this->url->link("extension/feed/merchant_feed/category", "user_token=" . $this->session->data["user_token"], true)];
        ...
        if (isset($this->request->post["feed_merchant_feed_category_items"])) {
            ...
            if ($this->config->get("feed_merchant_feed_category_items")) {
                ...
                $data["feed_merchant_feed_category_items"] = [];
            }
        }
        $data["header"] = $this->load->controller("common/header");
        $data["column_left"] = $this->load->controller("common/column_left");
    ...
    public function autocomplete()
    {
        ...
        if (isset($this->request->get["filter_text"])) {
            ...
            $filter_data = ["filter_text" => $filter_text, "start" => 0, "limit" => $limit];
            $results = $this->model_extension_feed_merchant_feed->getGoogleCategory($filter_data);
            foreach ($results as $result) {
                $json[] = ["google_id" => $result["google_id"], "text" => strip_tags(html_entity_decode($result["text"], ENT_QUOTES, "UTF-8"))];
            }
        }
        $this->response->addHeader("Content-Type: application/json");
        $this->response->setOutput(json_encode($json));
