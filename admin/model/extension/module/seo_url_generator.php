...
class ModelExtensionModuleSeoURLGenerator extends Model {
	...
	*/
	...
	public function deleteAllURL($query_key, $essence_id, $store_id, $language_id) {
		$this->stdelog->write(4, 'deleteURL() is called');
		
		$sql = "DELETE FROM `" . DB_PREFIX . "seo_url` WHERE `query` = '" . $this->db->escape($query_key) . "=" . (int)$essence_id . "' AND `store_id` = '" . (int)$store_id . "'";

		$query = $this->db->query($sql);
		
		$this->stdelog->write(4, $query, 'deleteAllURL() : $query');
	...
	public function isUnique($keyword, $query_key, $essence_id, $store_id) {
		...
		//	$sql_uniq = "DELETE FROM `" . DB_PREFIX . "seo_url` WHERE `keyword`='" . $this->db->escape($keyword) . "'";

		//	$this->db->query($sql_uniq);
