...
class ControllerCustomerCustomer extends Controller {
	...
	protected function getForm() {
		...
		// Inpost
		// $this->document->addScript('../catalog/view/javascript/inpost/inpost.js');
		// $this->document->addStyle('../catalog/view/javascript/inpost/inpost.css');
		...
		$countries = $this->model_localisation_country_delivery->getCountryDeliveries();
		$countrys = array();
	...
	protected function validateForm() {
		...
		if (!isset($this->request->get['customer_type']) && $this->request->get['customer_type'] == 3) {
			return true;
		}
		...
		if (isset($this->request->get['filter_name']) || isset($this->request->get['filter_email'])) {
			...
			foreach ($results as $result) {
				...
				$json[] = array(
					...
					'name'              => $customer_name, // strip_tags(html_entity_decode($result['name'], ENT_QUOTES, 'UTF-8')),
