...
class ControllerExtensionModuleXDSProductsCarousel extends Controller {
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			if (!isset($this->request->get['module_id'])) {
				$this->model_setting_module->addModule('xds_products_carousel', $this->request->post);
			} else {
		...
		if (!isset($this->request->get['module_id'])) {
			$data['breadcrumbs'][] = array(
				...
				'href' => $this->url->link('extension/module/xds_products_carousel', 'user_token=' . $this->session->data['user_token'], true)
			);
		} else {
			$data['breadcrumbs'][] = array(
				'text' => strip_tags($this->language->get('heading_title')),
				'href' => $this->url->link('extension/module/xds_products_carousel', 'user_token=' . $this->session->data['user_token'] . '&module_id=' . $this->request->get['module_id'], true)
			);
		}

		if (!isset($this->request->get['module_id'])) {
			$data['action'] = $this->url->link('extension/module/xds_products_carousel', 'user_token=' . $this->session->data['user_token'], true);
		} else {
			$data['action'] = $this->url->link('extension/module/xds_products_carousel', 'user_token=' . $this->session->data['user_token'] . '&module_id=' . $this->request->get['module_id'], true);
		}
		...
		if (isset($this->request->post['show_buttons'])) {
			$data['show_buttons'] = $this->request->post['show_buttons'];
		} elseif (!empty($module_info)) {
			$data['show_buttons'] = $module_info['show_buttons'];
		} else {
			$data['show_buttons'] = array('wishlist','compare','fastorder','qview');
		}
		...
		} elseif (!empty($module_info) && isset($module_info['cssclass'])) {
		...
		$this->response->setOutput($this->load->view('extension/module/xds_products_carousel', $data));
	}

	protected function validate() {
		if (!$this->user->hasPermission('modify', 'extension/module/xds_products_carousel')) {
