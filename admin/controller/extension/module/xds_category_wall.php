...
class ControllerExtensionModuleXDSCategoryWall extends Controller {
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			if (!isset($this->request->get['module_id'])) {
				$this->model_setting_module->addModule('xds_category_wall', $this->request->post);
			} else {
		...
		if (!isset($this->request->get['module_id'])) {
			$data['breadcrumbs'][] = array(
				...
				'href' => $this->url->link('extension/module/xds_category_wall', 'user_token=' . $this->session->data['user_token'], true)
			);
		} else {
			$data['breadcrumbs'][] = array(
				'text' => strip_tags($this->language->get('heading_title')),
				'href' => $this->url->link('extension/module/xds_category_wall', 'user_token=' . $this->session->data['user_token'] . '&module_id=' . $this->request->get['module_id'], true)
			);
		}

		if (!isset($this->request->get['module_id'])) {
			$data['action'] = $this->url->link('extension/module/xds_category_wall', 'user_token=' . $this->session->data['user_token'], true);
		} else {
			$data['action'] = $this->url->link('extension/module/xds_category_wall', 'user_token=' . $this->session->data['user_token'] . '&module_id=' . $this->request->get['module_id'], true);
		}
		...
		if (isset($this->request->post['allcat'])) {
			$data['allcat'] = $this->request->post['allcat'];
		} elseif (!empty($module_info)) {
			$data['allcat'] = $module_info['allcat'];
		} else {
			$data['allcat'] = '';
		}
		if (isset($this->request->post['catline'])) {
			$data['catline'] = $this->request->post['catline'];
		} elseif (!empty($module_info)) {
			$data['catline'] = $module_info['catline'];
		} else {
			$data['catline'] = '';
		}
    ...
    if (isset($this->request->post['click_action'])) {
			$data['click_action'] = $this->request->post['click_action'];
		} elseif (!empty($module_info)) {
			$data['click_action'] = $module_info['click_action'];
		} else {
			$data['click_action'] = '';
		}
    ...
    $this->load->model('catalog/category');
	  ...
	  if (isset($this->request->post['category'])) {
		...
		$this->response->setOutput($this->load->view('extension/module/xds_category_wall', $data));
	}

	protected function validate() {
		if (!$this->user->hasPermission('modify', 'extension/module/xds_category_wall')) {
