<?php
class ControllerExtensionPaymentPrzelewycard24 extends Controller {
    ...
    */
    public function __construct($registry) {
        ...
        $this->load->language('extension/payment/przelewycard24');
    ...
    public function index() {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
            $this->model_setting_setting->editSetting('payment_przelewycard24', $this->request->post);
            $this->session->data['success'] = $this->language->get('text_success');
            $this->response->redirect($this->makeUrl('extension/payment/przelewycard24'));
        }
        ...
        */
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href' => $this->makeUrl('extension/payment/przelewycard24')
        );
        ...
        $data['payment_przelewycard24_currency'] = $this->config->get('payment_przelewycard24_currency');

        $data['action'] = $this->makeUrl('extension/payment/przelewycard24');
        ...
        foreach ($this->post_data as $value) {
            if (isset($this->request->post['payment_przelewycard24_'.$value])) {
                $data['payment_przelewycard24_'.$value] = $this->request->post['payment_przelewycard24_'.$value];
            } else {
                $data['payment_przelewycard24_'.$value] = $this->config->get('payment_przelewycard24_'.$value);
            }
        }

        $payment_przelewycard24_name_val = $this->config->get('payment_przelewycard24_name');
        if(!isset($payment_przelewycard24_name_val)){
            $data['payment_przelewycard24_name'] = 'przelewy24.pl';
        }

        $data['header'] = $this->load->controller('common/header');
        $data['column_left'] = $this->load->controller('common/column_left');
        $data['footer'] = $this->load->controller('common/footer');
        $this->response->setOutput($this->load->view('extension/payment/przelewycard24', $data));
    }
    ...
    */
    ...
    protected function validate() {
        ...
        if (!$this->user->hasPermission('modify', 'extension/payment/przelewycard24')) {
            $this->error['warning'] = $this->language->get('error_permission');
        }
        
        if (!$this->request->post['payment_przelewycard24_key_order']) {
            $this->error['key_order'] = $this->language->get('error_key_order');
        }
        if (!$this->request->post['payment_przelewycard24_key_api']) {
            $this->error['key_api'] = $this->language->get('error_key_api');
        }
        if (!$this->request->post['payment_przelewycard24_key_crc']) {
