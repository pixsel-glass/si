...
class ControllerSaleOrder extends Controller {
	...
	protected function getList() {
		...
		foreach ($results as $result) {
			...
			$client_info = $this->model_customer_customer->getCustomer($result['customer_id']);
			if (!empty($client_info)) {
				if ($client_info['customer_type'] == 1 && !empty($client_info['company_name'])) {
					$customer_name = $client_info['company_name'] . '<br>' . $result['customer'];
				} else {
	...
	public function info() {
		...
		if ($order_info) {
			...
			$data['date_added'] = date("d.m.Y H:i:s", strtotime($order_info['date_added']));
			...
			if ($order_info['customer_id']) {
				...
				$this->load->model('customer/customer');
				$client_info = $this->model_customer_customer->getCustomer($order_info['customer_id']);
			...
			foreach ($totals as $total) {
				$data['totals'][] = array(
					'title' => $total['title'],
					'code' => $total['code'],
					'tax_text'  => $this->currency->format($this->tax->calc_tax($total['value']), $order_info['currency_code'], $order_info['currency_value']),
			...
			$data['pixsel_price_rate'] = $order_info['pixsel_price_rate'];
	...
	public function updatepayed() {
		if (isset($this->request->post['payed'])) {
			
			$order_id = $this->request->get['order_id'];
			$payed = $this->request->post['payed'];

			$this->db->query("UPDATE `" . DB_PREFIX . "order` SET payed = '" . (int)$payed . "' WHERE order_id = '" . (int)$order_id . "'");
