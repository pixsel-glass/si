...
class ControllerPdfexcelimportPdfexcelimport extends Controller {
    ...
    public function small_pdf($redirect=1, $from_site = 0){
      ...
      foreach ($products as $product) {
        ...
        $name = $product_info[2]['name'].' '.$category_info['name'];
        ...
        foreach ($product_options as $key => $value) {
           foreach ($value['product_option_value'] as $key2 => $value2) {
               if($value2['product_option_value_id']==$options[0]['product_option_value_id']){
                 $code = ' ( '.trim($value2['option_code_1']).' )';
               }
           }
        }
        ...
        $pdf->SetFontSize(8);
        $pdf->MultiCell('',7,iconv('utf-8', 'windows-1251', $name),0,'L',0);
        $pdf->Ln(1);
        $pdf->SetFontSize(5.5);
        $pdf->MultiCell('',7,iconv('utf-8', 'windows-1251', $nameoptions),0,'L',0);
        }
      }
    ...
    public function reload_pdf(){
        ...
        set_time_limit (60);
        ini_set('memory_limit', '5512M');
        $this->document->setTitle('Прайс-листы');
        ...
        $pdf->Write(5,iconv('utf-8', 'windows-1251',"т. +38 096 425 09 07"));
        //$pdf->Ln( 12 );
        $pdf->SetXY($left_header,60);
        $pdf->Write(5,iconv('utf-8', 'windows-1251','т. +38 099 425 09 07'));
        $pdf->SetXY($left_header,75);
        $pdf->Write(5,iconv('utf-8', 'windows-1251','т. +38 093 425 09 07'));
        $pdf->SetXY($left_header,90);
        $pdf->Write(5,iconv('utf-8', 'windows-1251','info@pixsel.com.ua'));

        $src = $_SERVER['DOCUMENT_ROOT'].'/image/insta.png';
        $pdf->Image($src,$left_header,105,18);
        $pdf->SetXY($left_header + 20,110);
        $pdf->Write(5,iconv('utf-8', 'windows-1251','pixsel_glass'));
        //---------header---------------------
        ...
        $pdf->SetFontSize(7);
        $pdf->SetTextColor(0,0,0);
        $width = array(10,50,60,142,55);
        $column_height = 20;
        $name_price_1 = 'Каленое стекло 9H';
        $name_price_2 = 'Гибрид Глянец 9H / Мат';
        ...
        foreach ($prices_dopin as $key => $value) {
            ...
            if($count_f==1){
              $pdf->Cell(($width[4]*2) + ($merge_price_first*2),$column_height,iconv('utf-8', 'windows-1251',$customer_group_dopin['name']),1,0,'C',0);
              $count_f = 2;
            }else{
              $pdf->Cell(($width[4]*2) + ($merge_price*2),$column_height,iconv('utf-8', 'windows-1251',$customer_group_dopin['name']),1,0,'C',0); 
            }
        }
        $pdf->Ln();
        ...
        if($customer_group['prices']['prices_code']=='1'){
           $pdf->Cell($width[1],$column_height,iconv('utf-8', 'windows-1251','Код товара'),1,0,'C',0);
        }else{
           $merge_name = $merge_name + $width[1];
        }
        if($customer_group['prices']['prices_articul']=='1'){
           $pdf->Cell($width[1],$column_height,iconv('utf-8', 'windows-1251','Артикул товара'),1,0,'C',0);
        }else{
           $merge_name = $merge_name + $width[1];
        }
        if($customer_group['prices']['prices_class']=='1'){
           $pdf->Cell($width[1],$column_height,iconv('utf-8', 'windows-1251','Класс товара'),1,0,'C',0);
        }else{
           $merge_name = $merge_name + $width[1];
        }
        if($customer_group['prices']['prices_img']=='1'){
           $pdf->Cell($width[2],$column_height,iconv('utf-8', 'windows-1251','Картинка товара'),1,0,'C',0);
        }else{
           $merge_name = $merge_name + $width[2];
        }

        $pdf->Cell($width[3] + $merge_name,$column_height,iconv('utf-8', 'windows-1251','Наименование'),1,0,'C',0);
        ...
        foreach ($prices_ids as $key => $value) {
            ...
            if($category_line!=''){
               $results_products = $this->model_catalog_category->getProductsClearPrice($category_line, $sort_by, $prices_model1, $manufacturer_id, $sklad);
            }
            ...
            if(count($results_products)>0){
        	  ...
        	  foreach ($results_products as $key => $value) {
                    ...
                    if($customer_group['prices']['prices_class']=='1'){
                        $pdf->Cell($width[1],$column_height,iconv('utf-8', 'windows-1251',$value['manufacturer_name']),1,0,'C',0);
                        $merge_column_height_name_count = $merge_column_height_name_count + 1;
                    }
                    if($customer_group['prices']['prices_img']=='1'){
                       $x=$pdf->GetX();
                    ...
                    $x=$pdf->GetX();
                    $y=$pdf->GetY();
                    $sa = iconv('utf-8', 'windows-1251//IGNORE',trim($value['name']));
                	            ...
                	            if($value_o1['option_id']=='16'){
                                    foreach ($value_o1['product_option_value'] as $key_o2 => $value_o2) {
                                        if(!in_array($valuedopin, $prices_dopin_poask)){
                                          ...
                                          $price = iconv('utf-8', 'windows-1251//IGNORE','по запросу');  
                                        }
    ...
    public function reload_excel(){
        ...
        $src = $_SERVER['DOCUMENT_ROOT'].'/image/logoexcel.png';
        $objPHPExcel->insertImg('Pixsel защитные стекла', $src, 0, 10, 120, 120, 'A1');                     
        $objPHPExcel->mergeCells(9,1,$column_merge,1);
        $objPHPExcel->mergeCells(9,2,$column_merge,2);
        $objPHPExcel->setValue(9,2,'т. +38 096 425 09 07');
        $objPHPExcel->mergeCells(9,3,$column_merge,3);
        $objPHPExcel->setValue(9,3,'т. +38 099 425 09 07');
        $objPHPExcel->mergeCells(9,4,$column_merge,4);
        $objPHPExcel->setValue(9,4,'т. +38 093 425 09 07');
        $objPHPExcel->mergeCells(9,5,$column_merge,5);
        $objPHPExcel->setValue(9,5,'info@pixsel.com.ua');
        //$objPHPExcel->insertLink(9,5,'mailto:info@pixsel.com.ua');
        $objPHPExcel->mergeCells(9,6,$column_merge,6);
        $objPHPExcel->setValue(9,6,'       pixsel_glass');
        $objPHPExcel->mergeCells(9,7,$column_merge,7);

        $src = $_SERVER['DOCUMENT_ROOT'].'/image/insta.png';
        $objPHPExcel->insertImg('Pixsel инстаграмм', $src, 0, 4, 18, 18, 'J6');
        ...
        if($customer_group['prices']['prices_code']=='1'){
           ...
           $objPHPExcel->setValue($column,($row+1),'Код товара');
        ...
        if($customer_group['prices']['prices_articul']=='1'){
           $objPHPExcel->setValue($column,$row,'');
           $objPHPExcel->setValue($column,($row+1),'Артикул товара');
           $objPHPExcel->setColumnWidth($column, "8");
           $column = $column + 1;
        }else{
           $merge_name = $merge_name + 1;
        ...
        if($customer_group['prices']['prices_class']=='1'){
           $objPHPExcel->setValue($column,$row,'');
           $objPHPExcel->setValue($column,($row+1),'Класс товара');
           $objPHPExcel->setColumnWidth($column, "8");
           $column = $column + 1;
        }else{
           $merge_name = $merge_name + 1;
        ...
        if($customer_group['prices']['prices_img']=='1'){
           $objPHPExcel->setValue($column,$row,'');
           $objPHPExcel->setValue($column,($row+1),'Картинка товара');
        ...
        $objPHPExcel->setValue($column,$row,'');
        $objPHPExcel->setValue($column,($row+1),'Наименование');
        ...
        foreach ($prices_dopin as $key => $value) {
                ...
                if($count_f==1){
                    ...
                    $objPHPExcel->setValue($column_work,$row,$customer_group_dopin['name']);
        ...
        foreach ($prices_ids as $key => $value) {
            ...
            if(count($results_products)>0){
        	    ...
        	    foreach ($results_products as $key => $value) {
                    ...
                    $objPHPExcel->setValue($column,$row,$value['name']);
                    $column = $column + $merge_name + 1;
    ...
    public function edit(){
        ...
        $data['categories'] = $categories;
		    ...
		    $data['breadcrumbs'][] = array(
		    	'text' => 'Редактирование настроек прайс-листа',
		    	'href' => $this->url->link('pdfexcelimport/pdfexcelimport', 'user_token=' . $this->session->data['user_token'], true)
		    );
	...
	public function index() {
		...
		if(isset($this->request->get['success']) and $this->request->get['success']==1){
			$data['success'] = 'Excel прайс-лист для группы обновлен';
		}
		if(isset($this->request->get['success']) and $this->request->get['success']==2){
			$data['success'] = 'Pdf прайс-лист для группы обновлен';
		}
