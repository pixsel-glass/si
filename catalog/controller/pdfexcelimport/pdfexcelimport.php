...
class ControllerPdfexcelimportPdfexcelimport extends Controller {
    ...
    public function small_pdf($redirect=1, $from_site = 0){    
      ...
      foreach ($products as $product) {
        ...
        $product_info = $this->getProductDescriptions($product['product_id']);
        ...
        $name = $product_info[$language_sticker]['name'].' '.$category_info['name'];

        $code = '';
        $options = $this->model_checkout_order->getOrderOptions($order_id, $product['order_product_id']);
        $product_options = $this->getProductOptionsLanguage($product['product_id'],$language_sticker);

        foreach ($product_options as $key => $value) {
            $option_values = $this->getOptionValuesMs($value['option_id'], $language_sticker);
    ...
    public function getOptionValuesMs($option_id, $language_id) {
        ...
        public function getProductOptionsLanguage($product_id,$language_id) {
            $product_option_data = array();

            $product_option_query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "product_option` po LEFT JOIN `" . DB_PREFIX . "option` o ON (po.option_id = o.option_id) LEFT JOIN `" . DB_PREFIX . "option_description` od ON (o.option_id = od.option_id) WHERE po.product_id = '" . (int)$product_id . "' AND od.language_id = '" . (int)$language_id . "' ORDER BY o.sort_order ASC");
        ...
        'pixsel_price_material' => $product_option_value['pixsel_price_material'],
    ...
    public function reload_pdf(){
        ...
        $pdf->Write(5,iconv('utf-8', 'windows-1251',"т. +38 099 795 09 07"));
        ...
        $column_height = 20;
        $name_price_1 = 'Гартоване скло 9H';
        $name_price_2 = 'Гібрид Глянець 9H / Мат';
        ...
        if($customer_group['prices']['prices_code']=='1'){
           $pdf->Cell($width[1],$column_height,iconv('utf-8', 'windows-1251','Код товару'),1,0,'C',0);
        }else{
           $merge_name = $merge_name + $width[1];
        }
        if($customer_group['prices']['prices_articul']=='1'){
           $pdf->Cell($width[1],$column_height,iconv('utf-8', 'windows-1251','Артикул товару'),1,0,'C',0);
        }else{
           $merge_name = $merge_name + $width[1];
        }
        if($customer_group['prices']['prices_class']=='1'){
           $pdf->Cell($width[1],$column_height,iconv('utf-8', 'windows-1251','Клас товару'),1,0,'C',0);
        }else{
           $merge_name = $merge_name + $width[1];
        }
        if($customer_group['prices']['prices_img']=='1'){
           $pdf->Cell($width[2],$column_height,iconv('utf-8', 'windows-1251','Зображення товару'),1,0,'C',0);
        }else{
           $merge_name = $merge_name + $width[2];
        }

        $pdf->Cell($width[3] + $merge_name,$column_height,iconv('utf-8', 'windows-1251','Найменування'),1,0,'C',0);
    ...
    public function reload_excel(){
        ...
        $objPHPExcel->setValue(9,2,'т. +38 099 795 09 07');
        $objPHPExcel->mergeCells(9,3,$column_merge,3);
        $objPHPExcel->setValue(9,3,'');
        $objPHPExcel->mergeCells(9,4,$column_merge,4);
        $objPHPExcel->setValue(9,4,'');
        ...
        $objPHPExcel->setValue(9,6,'       pixsel.glass');
        ...
        if($customer_group['prices']['prices_code']=='1'){
           ...
           $objPHPExcel->setValue($column,($row+1),'Код товару');
        ...
        if($customer_group['prices']['prices_articul']=='1'){
           $objPHPExcel->setValue($column,$row,'');
           $objPHPExcel->setValue($column,($row+1),'Артикул товару');
           $objPHPExcel->setColumnWidth($column, "8");
           $column = $column + 1;
        }else{
           $merge_name = $merge_name + 1;
        ...
        if($customer_group['prices']['prices_class']=='1'){
           $objPHPExcel->setValue($column,$row,'');
           $objPHPExcel->setValue($column,($row+1),'Клас товару');
           $objPHPExcel->setColumnWidth($column, "8");
           $column = $column + 1;
        }else{
           $merge_name = $merge_name + 1;
        ...
        if($customer_group['prices']['prices_img']=='1'){
           $objPHPExcel->setValue($column,$row,'');
           $objPHPExcel->setValue($column,($row+1),'Зображення товару');
        ...
        $objPHPExcel->setValue($column,$row,'');
        $objPHPExcel->setValue($column,($row+1),'Найменування');
