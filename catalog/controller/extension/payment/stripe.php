...
class ControllerExtensionPaymentStripe extends Controller {
	...
	public function index() {
		...
		// Unset session variables not used for normal payments
		unset($this->session->data['finalized_by_webhook']);
		...
		// Load pop-up if enabled
		if ($settings['use_popup'] && empty($this->request->post['load_popup'])) {
			$data['use_popup'] = true;
			$output = $this->renderTemplate($data);
			return $output;
		} else {
			$data['use_popup'] = false;
		}
		...
		if (!$data['use_stripe_checkout']) {
			...
			if ($settings['charge_mode'] == 'authorize') {
				...
				$curl_data['payment_method_options']['amazon_pay'] = array('capture_method' => 'manual');
			...
			$payment_intent_unusable = (!empty($data['payment_intent']) && empty($data['payment_intent']['error']) && !in_array($data['payment_intent']['status'], array('canceled', 'requires_confirmation', 'requires_payment_method')));
			...
			// Create new PaymentIntent if necessary
			if (empty($data['payment_intent']) || !empty($data['payment_intent']['error']) || $payment_intent_unusable || $subscription_switching) {
				...
				if ($plans) {
					if ($currency == 'USD') {
						$curl_data['payment_method_types'] = array('card', 'us_bank_account');
					} else {
		...
		if (!empty($this->request->post['load_popup'])) {
			echo $output;
		} else {
			return $output;
		}
	...
	private function renderTemplate($data) {
		...
		if (is_file($template_file)) {
			...
			if (version_compare(VERSION, '4.0', '>=')) {
				...
				$output = str_replace($data['settings']['extension_route'] . '/', $data['settings']['extension_route'] . $separator, $output);
			}
		} else {
			$output = 'Error loading template file: ' . $template_file;
		}
	...
	//==============================================================================
	public function getSubscriptionPlans($settings = array(), $order_info = array()) {
		if (empty($settings)) {
		...
		if (version_compare(VERSION, '2.0', '<')) {
			...
			$profile_recurring_subscription = 'subscription';
		}
		...
		foreach ($plans as $index => $plan) {
			...
			foreach ($cart_products as $product) {
				...
				if ($key != $plans[$index]['product_key']) {
					$this->cart->remove(version_compare(VERSION, '2.1', '<') ? $key : $product['cart_id']);
				}
			}
			...
			foreach ($shipping_methods as $shipping_method) {
				...
				foreach ($quote['quote'] as $q) {
					...
					$plans[$index]['shipping_cost'] = $q['cost'];
					$plans[$index]['taxed_shipping_cost'] = $this->tax->calculate($q['cost'], $q['tax_class_id']);
	...
	public function createCheckoutSession() {
		...
		if (empty($settings['account_id']) || empty($settings['refresh_token'])) {
			echo json_encode(array('error_message' => 'Error: extension is not connected'));
			return;
		}
		...
		if (empty($this->session->data['order_id'])) {
			echo json_encode(array('error_message' => $settings['checkout_no_order_id_' . $language]));
			return;
		}
		...
		$checkout_data = array(
			...
			'line_items'				=> array(),
			'success_url'				=> $this->url->link($settings['extension_route'] . $separator . 'checkoutComplete', version_compare(VERSION, '4.0', '<') ? '' : 'language=' . $language, 'SSL'),
			'cancel_url'				=> $this->url->link('checkout/onepcheckout', version_compare(VERSION, '4.0', '<') ? '' : 'language=' . $language, 'SSL'),
		...
		if ($negative_line_item && empty($plans)) {
			...
			foreach ($order_products as $product) {
				...
				if (version_compare(VERSION, '2.0', '<')) {
					$product['profile'] = array('profile_id' => (!empty($product['profile_id']) ? $product['profile_id'] : 0), 'name' => (!empty($product['profile_name']) ? $product['profile_name'] : ''));
					$profile_recurring_subscription = 'profile';
				} elseif (version_compare(VERSION, '4.0', '<')) {
					$profile_recurring_subscription = 'recurring';
				} else {
	...
	public function updatePaymentIntent() {
		...
		// Check for second payment attempt on the same order, and Collect Payment Later is not being used
	...
	public function confirmPaymentIntent() {
		...
		$hr = '<hr style="color: #888; margin: 5px;">';
	...
	public function finalizePayment($payment_intent_id = '', $finalized_by_webhook = false) {
		...
		if (!empty($this->session->data['finalized_by_webhook'])) {
			$finalized_by_webhook = true;
			unset($this->session->data['finalized_by_webhook']);
		}
		...
		// Complete order immediately if this is a second payment attempt, and Collect Payment Later is not being used
		if (empty($this->session->data['payment_in_process'])) {
			$order_history_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_history WHERE order_id = " . (int)$order_info['order_id'] . " AND `comment` LIKE '%Stripe Payment ID%'");
		...
		if ($finalized_by_webhook) {
			...
			if (!empty($settings['merge_subscriptions']) && $plans) {
				...
				foreach ($plans as $index => $temp_plan) {
					$plans[$index]['items'] = array(
						array(
							'plan'		=> $temp_plan['id'],
							'quantity'	=> $temp_plan['quantity'],
							'metadata'	=> array(
			...
			foreach ($plans as $index => $plan) {
				...
				$plans[$index]['total_plan_cost'] = $total_plan_cost;
				$plans[$index]['subscription_response'] = $subscription_response;
			}
		}
		...
		$subscription_payment_type = (!empty($charge['payment_method_details']['type'])) ? $charge['payment_method_details']['type'] : '';

		foreach ($plans as $plan) {
			if (!empty($plan['subscription_response'])) {
				$subscription_response = $plan['subscription_response'];

				if (!empty($subscription_response['default_payment_method'])) {
					$subscription_payment_method = $this->curlRequest('GET', 'payment_methods/' . $subscription_response['default_payment_method']);

					if (!empty($subscription_payment_method['type'])) {
						$subscription_payment_type = $subscription_payment_method['type'];
						$comment .= $strong . 'Payment Method:</strong>' . $subscription_payment_method['type'] . '<br>';
					}
				}
			}
		...
		if (!empty($charge)) {
			...
			if ($charge['payment_method_details']['type'] == 'card') {
				...
				if (!empty($card['wallet']['type']) && $card['wallet']['type'] == 'apple_pay') {
					$comment .= $strong . 'Payment Method:</strong>apple_pay<br>';
		...
		// Override extension title if set
		if (!empty($settings['override_extension_title']) && !empty($charge['payment_method_details']['type']) && $charge['payment_method_details']['type'] != 'card') {
			$payment_method = ucwords(str_replace('_', ' ', $charge['payment_method_details']['type']));
			if (version_compare(VERSION, '4.0.2.0', '<')) {
				$this->db->query("UPDATE `" . DB_PREFIX . "order` SET payment_method = '" . $this->db->escape($payment_method) . "' WHERE order_id = " . (int)$order_info['order_id']);
			} else {
				$payment_method_array = array('code' => $this->name . '.' . $this->name, 'name' => $payment_method);
				$this->db->query("UPDATE `" . DB_PREFIX . "order` SET payment_method = '" . $this->db->escape(json_encode($payment_method_array)) . "' WHERE order_id = " . (int)$order_info['order_id']);
			}
		}

		// Strip HTML from payment method title on the order
		$payment_method = $this->db->query("SELECT * FROM `" . DB_PREFIX . "order` WHERE order_id = " . (int)$order_info['order_id'])->row['payment_method'];

		if (version_compare(VERSION, '4.0.2.0', '<')) {
			$this->db->query("UPDATE `" . DB_PREFIX . "order` SET payment_method = '" . $this->db->escape(strip_tags($payment_method)) . "' WHERE order_id = " . (int)$order_info['order_id']);
		} else {
			$payment_method_object = json_decode($payment_method, true);
			$payment_method_array = array('code' => $this->name . '.' . $this->name, 'name' => strip_tags($payment_method_object['name']));
			$this->db->query("UPDATE `" . DB_PREFIX . "order` SET payment_method = '" . $this->db->escape(json_encode($payment_method_array)) . "' WHERE order_id = " . (int)$order_info['order_id']);
		}
		...
		foreach ($plans as $plan) {
			if (!empty($plan['subscription_response']['latest_invoice'])) {
				$invoice_response = $this->curlRequest('GET', 'invoices/' . $plan['subscription_response']['latest_invoice']);
				...
				if (!empty($invoice_response['payment_intent'])) {
					...
					if (!empty($payment_intent_response['error'])) {
						...
						$json['client_secrets'][] = $payment_intent_response['client_secret'];
						$json['payment_type'] = $subscription_payment_type;
					}
				}
			}
		}
	...
	public function paymentComplete() {
		...
		$order_history_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_history WHERE `comment` LIKE '%Captured:%' AND order_id = " . (int)$order_id);
		...
		if (!$order_history_query->num_rows) {
			...
			if (in_array($payment_intent['status'], $incomplete_statuses)) {
				$this->response->redirect($this->url->link('checkout/checkout', version_compare(VERSION, '4.0', '<') ? '' : 'language=' . $language, 'SSL'));
			}
			...
			if (!empty($settings['initial_status_id'])) {
				...
				$this->db->query("UPDATE oc_order SET payed='1', payed_with='stripe' WHERE order_id = '" . $order_id . "'");
		...
		$this->response->redirect($this->url->link('checkout/success', version_compare(VERSION, '4.0', '<') ? '' : 'language=' . $language, 'SSL'));
	}
			...
			if ($details) {
				...
				if ($country_query->num_rows) {
					...
					if (version_compare(VERSION, '4.1', '<')) {
						...
						$country_name = $this->db->query("SELECT * FROM " . DB_PREFIX . "country_description WHERE country_id = " . (int)$country_id . " AND language_id = " . (int)$this->config->get('config_language_id'))->row['name'];
					}
				}
				...
				if ($zone_query->num_rows) {
					...
					if ($zone_query->num_rows) {
						...
						if (version_compare(VERSION, '4.1', '<')) {
							...
							$zone_name = $this->db->query("SELECT * FROM " . DB_PREFIX . "zone_description WHERE zone_id = " . (int)$zone_id . " AND language_id = " . (int)$this->config->get('config_language_id'))->row['name'];
						}
					}
				}
	...
	public function webhook() {
		...
		if ($event['type'] == 'cash_balance.funds_available') {
			...
			$this->addOrderHistory($order_id, $order_status_id, $comment, false);
			...
			if (empty($customer_response['error'])) {
				...
				$customer_description = (!empty($customer_response['description'])) ? $customer_response['description'] : '';
				$customer_email = (!empty($customer_response['email'])) ? $customer_response['email'] : '';
				$message = 'Subscription: ' . $product_name . ' (' . $webhook['id'] . ')<br>Customer: ' . $customer_description . ' ' . $customer_email;
			...
			if ($settings['order_address'] == 'stripe') {
				...
				if (version_compare(VERSION, '4.1', '<')) {
					...
					$zone_description = $this->db->query("SELECT * FROM " . DB_PREFIX . "zone_description WHERE zone_id = " . (int)$zone_id . " AND language_id = " . (int)$this->config->get('config_language_id'))->row;
					$opencart_address['zone'] = (!empty($zone_description['name'])) ? $zone_description['name'] : '';
				}
				...
				if (version_compare(VERSION, '4.1', '<')) {
					...
					$country_description = $this->db->query("SELECT * FROM " . DB_PREFIX . "country_description WHERE country_id = " . (int)$country_id . " AND language_id = " . (int)$this->config->get('config_language_id'))->row;
					$opencart_address['country'] = (!empty($country_description['name'])) ? $country_description['name'] : '';
				}
			...
			foreach ($webhook['lines']['data'] as $line) {
				...
				if (empty($line['plan'])) {
					...
					$cost = $line['amount'] / $line_decimal_factor;
					$subtotal += $cost;
					...
					$product_data[] = array(
						...
						'price'			=> ($cost / $line['quantity']),
						'total'			=> $cost,
						'tax'			=> $this->tax->getTax($cost, $product['tax_class_id']),
			...
			if (!empty($original_order_info)) {
				if ((strtotime($now_query->row['NOW()']) - strtotime($original_order_info['date_added'])) < 82800) {
					...
					if (!empty($webhook['payment_intent'])) {
						...
						$new_description = $this->{'model_' . str_replace('/', '_', $settings['extension_route'])}->replaceShortcodes($settings['transaction_description'], $original_order_info);
			...
			if (!empty($webhook['payment_intent'])) {
				...
				$new_description = $this->{'model_' . str_replace('/', '_', $settings['extension_route'])}->replaceShortcodes($settings['transaction_description'], $order_info);

				$this->curlRequest('POST', 'payment_intents/' . $webhook['payment_intent'], array('description' => $new_description));
			}

			if (!empty($webhook['charge'])) {
				$charge = $this->curlRequest('GET', 'charges/' . $webhook['charge']);
	...
	//==============================================================================
	// loadPaymentMethodMessaging()
	//==============================================================================
	public function loadPaymentMethodMessaging() {
		$settings = $this->getSettings();

		if (!empty($this->request->post['product_id'])) {
			$product = $this->db->query("SELECT * FROM " . DB_PREFIX . "product WHERE product_id = " . (int)$this->request->post['product_id'])->row;
			$amount = $this->tax->calculate($product['price'], $product['tax_class_id'], $this->config->get('config_tax'));
		} else {
			$amount = $this->cart->getTotal();
		}

		$store_country_code = $this->db->query("SELECT * FROM " . DB_PREFIX . "country WHERE country_id = " . (int)$this->config->get('config_country_id'))->row['iso_code_2'];
	...
	// addOrderHistory()
	//==============================================================================
	private function addOrderHistory($order_id, $order_status_id, $comment = '', $notify = true, $override = false) {
		$this->load->model('checkout/order');
		if (version_compare(VERSION, '4.0', '<')) {
