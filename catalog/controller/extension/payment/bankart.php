...
final class ControllerExtensionPaymentBankart extends Controller
{
    ...
    public function confirm()
    {
        ...
        try {
            ...
            $extraData =  array_merge(/*$this->extraData3DS(),*/ ["userField1" => $instalments], ["platform" => Client::PLATFORM]);

            $transaction->setMerchantTransactionId($this->encodeOrderId($this->session->data['order_id'])) //was setTransactionId
            ...
            //Set the 3DS object
		$threeDSdata = new \Bankart\Client\Data\ThreeDSecureData();
		...
		if ($this->getConfig('cc_force_challenge_' . $cardType) ==  '1') {
        ...
        $customer_info = $this->model_account_customer->getCustomer($this->order['customer_id']);

        if (!empty($customer_info) && isset($customer_info['date_added'])) {
            $date_added = (new DateTime($customer_info['date_added']))->format('Y-m-d');
            $threeDSdata->setCardholderAccountDate($date_added);
        }
		...
		*/
        ...
        if (!empty($customer_id)) {
            ...
            $payment_address1 = $this->order['payment_address_1'];
            $payment_address2 = $this->order['payment_address_2'];
            $payment_city = $this->order['payment_city'];

            $query = $this->db->query("
                SELECT MAX(date_added) as accLastChange 
                FROM " . DB_PREFIX . "order 
                WHERE customer_id = '" . (int)$customer_id . "'
                AND (
                    shipping_address_1 != '" . $shipping_address_1 . "'
                    OR shipping_address_2 != '" . $shipping_address_2 . "'
                    OR shipping_city != '" . $shipping_city . "'
                    OR shipping_country != '" . $shipping_country . "'
                    OR payment_address_1 != '" . $payment_address1 . "'
                    OR payment_address_2 != '" . $payment_address2 . "'
                    OR payment_city != '" . $payment_city . "'
                )
            ");

            $accLastChange = $query->row['accLastChange'] ? date('Y-m-d', strtotime($query->row['accLastChange'])) : null;        
            $threeDSdata->setCardholderAccountLastChange($accLastChange);
            ...
            $purchases = $query->row['purchases'];
            $threeDSdata->setPurchaseCountSixMonths((int)$purchases);
            ...
            $query = $this->db->query("
                SELECT MIN(date_added) as firstUseShippingAddress 
                FROM " . DB_PREFIX . "order 
                WHERE customer_id = '" . (int)$customer_id . "'
                AND shipping_address_1 = '" . $shipping_address_1 . "' 
                AND shipping_address_2 = '" . $shipping_address_2 . "' 
                AND shipping_city = '" . $shipping_city . "'
                AND shipping_country = '" . $shipping_country . "'
            ");

            $firstUseShippingAddress = $query->row['firstUseShippingAddress'] ? date('Y-m-d', strtotime($query->row['firstUseShippingAddress'])) : null;        
            $threeDSdata->setShippingAddressFirstUsage($firstUseShippingAddress);
        }
...
*/
        ...
        $orderId = $this->decodeOrderId($callbackResult->getMerchantTransactionId());
        $this->order = $this->model_checkout_order->getOrder($orderId);
        ...
        if(in_array($callbackResult->getTransactionType(), [CallbackResult::TYPE_PREAUTHORIZE, CallbackResult::TYPE_DEBIT])) {
            ...
            if (json_decode(json_encode($callbackResult->getPaymentMethod(), true)) != 'FlikPayments') {
                // for PHP 5.6
                ...
                $merchantMetaData = json_decode(json_encode($callbackResult->getMerchantMetaData()), true);
                $instalments = isset($merchantMetaData['instalments']) ? $merchantMetaData['instalments'] : null;

                if($instalments > 1) {
                    $orderHistoryComments[] = 'Instalments: ' . $instalments;
                }   
            } else {
                $orderHistoryComments[] = 'Card Type: ' . json_decode(json_encode($callbackResult->getPaymentMethod()), true);
                $customer = $callbackResult->getCustomer();
                $customerExtraData = $customer->getExtraData();

                if (array_key_exists('alias', $customerExtraData)) {
                    $orderHistoryComments[] = 'Alias: ' . json_encode($customerExtraData['alias']);
                }
            }
        }
