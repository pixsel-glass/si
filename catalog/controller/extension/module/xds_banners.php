...
class ControllerExtensionModuleXDSBanners extends Controller {
    ...
    if ($setting['type'] != 'single_banner') {
      ...
      if ($setting['animation_in'] || $setting['animation_in']) {
        $this->document->addStyle('catalog/view/theme/' . $this->config->get('theme_frame_directory') . '/stylesheet/animate.css');
      }
    }
    ...
    $data['lazyload_imgs'] = $setting['lazyload'];
	    ...
	    $height = $result['img_height'][$language_id] ? $result['img_height'][$language_id] : 200;
			...
			$data['banners'][] = array(
				...
				'src_holder'	=> $this->model_tool_image->resize('catalog/frametheme/src_holder.png', $width, $height),
				'img_width' 	=> $width . 'px',
    ...
    $data['title'] = html_entity_decode($setting['title'][$language_id], ENT_QUOTES, 'UTF-8');
		...
		// if ($setting['responsive_base'] == 'parent') {
		// 	$data['responsive_base'] = $setting['responsive_base'];
		// } elseif ($setting['responsive_base'] == 'custom') {
		// 	$data['responsive_base'] = $setting['responsive_base_selector'];
		// } else {
		// 	$data['responsive_base'] = '';
		// }
    ...
    $data['module'] = $module++;
		...
		return $this->load->view('extension/module/xds_banners', $data);
	}
}
