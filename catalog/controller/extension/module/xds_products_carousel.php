...
class ControllerExtensionModuleXDSProductsCarousel extends Controller {
    ...
    if (isset($ft_settings['t1_catalog_mode_hide_price']) && !empty($ft_settings['t1_catalog_mode_hide_price'])) {
      ...
      $data['hide_price'] = false;
		...
		if (isset($setting['cssclass']) && !empty($setting['cssclass'])) {
		...
		if ($setting['module_type'] == 'autofeatured') {
			...
			foreach ($results as $result) {
				...
				if (!($hide_noimage_products & !$result['image'])) {
					if ($setting['module_type'] == 'models') {
					    ...
					    $cat3info = $this->model_catalog_category->getCategory($cat3id);
					    $cat2id = $cat3info['parent_id'];
					    $cat2info = $this->model_catalog_category->getCategory($cat2id);
					    $cat1id = $cat2info['parent_id'];
						}

						if (!empty($cat3img)) {
							$image = $this->model_tool_image->resize($cat3img, $setting['width'], $setting['height']);
							if ($hd_imgs) {
								$image2x = $this->model_tool_image->resize($cat3img, $setting['width']*2, $setting['height']*2);
								$image3x = $this->model_tool_image->resize($cat3img, $setting['width']*3, $setting['height']*3);
								$image4x = $this->model_tool_image->resize($cat3img, $setting['width']*4, $setting['height']*4);
							}
						...
						$data['products'][] = array(
							...
							'href'        => $this->url->link('product/category', 'path=' . $cat1id . '_' . $cat2id . '_' . $cat3id)
						);						
					} else {
						$data['products'][] = array(
							'product_id'  => $result['product_id'],
