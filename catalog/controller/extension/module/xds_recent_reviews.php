...
class ControllerExtensionModuleXDSRecentReviews extends Controller {
    ...
    $results = $this->model_extension_theme_frame->getLastReviews($setting['limit'], $setting['rating']);
    ...
    foreach ($results as $result) {
      ...
      $data['reviews'][] = array(
        ...
        'product_href'  => $this->url->link('product/product', '&product_id=' . $result['product_id']),
				...
				'thumb_holder'    => $this->model_tool_image->resize('catalog/frametheme/src_holder.png', $setting['image_width'], $setting['image_height']),
        'product_image' => $image,
		...
		if ($setting['sort'] && !empty($data['reviews'])) {
			shuffle($data['reviews']);
		}

    $data['module'] = $module++;

		return $this->load->view('extension/module/xds_recent_reviews', $data);
	}
}
