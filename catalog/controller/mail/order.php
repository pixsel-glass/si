...
class ControllerMailOrder extends Controller {
	...
	public function index(&$route, &$args) {
		...
		} else if (!empty($order_info['comment'])) {
	...
	public function add($order_info, $order_status_id, $comment, $notify) {
		...
		$data['text_order_detail'] = html_entity_decode($language->get('text_order_detail'), ENT_QUOTES, 'UTF-8');
		...
		$data['text_client'] = $language->get('text_client');
		...
		$data['logo'] = $order_info['store_url'] . 'image/' . str_replace('.svg', '.png', $this->config->get('config_logo'));
		...
		$data['shipping_method'] = str_replace('alt=', 'width="40" alt=', str_replace('"image/', '"https://' . $_SERVER['SERVER_NAME'] . '/image/', $order_info['shipping_method']));
		...
		$data['shipping_city'] = ( !empty($order_info['parcelLocker']) ? $order_info['parcelLocker'] . ', ' . $order_info['shipping_city'] : $order_info['shipping_city'] );
		...
		foreach ($order_products as $order_product) {
			...
			if ($this->customer->isLogged() && $data['pixsel_tax_status'] || !$this->config->get('config_customer_price') && $data['pixsel_tax_status']) {
				$tax_price = $this->currency->format($this->tax->calc_tax($order_product['price']) + ($this->config->get('config_tax') ? $order_product['tax'] : 0), $order_info['currency_code'], $order_info['currency_value'], false);
			} else {
				$tax_price = false;
			}

			if ($this->customer->isLogged() && $data['pixsel_tax_status'] || !$this->config->get('config_customer_price') && $data['pixsel_tax_status']) {
				$tax_total = $this->currency->format($this->tax->calc_tax($order_product['total']) + ($this->config->get('config_tax') ? ($order_product['tax'] * $order_product['quantity']) : 0), $order_info['currency_code'], $order_info['currency_value'], false);
			} else {
				$tax_total = false;
			}
		...
		foreach ($order_totals as $order_total) {
			...
			$data['totals'][] = array(
				...
				'tax_text'  => $this->currency->format($this->tax->calc_tax($order_total['value']), $order_info['currency_code'], $order_info['currency_value']),
