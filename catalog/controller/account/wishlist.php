		...
		foreach ($results as $result) {
			...
			if ($product_info) {
				...
				if ($this->customer->isLogged() && $data['pixsel_tax_status'] || !$this->config->get('config_customer_price') && $data['pixsel_tax_status']) {
					$tax_price = $this->currency->format($this->tax->calc_tax($product_info['price']), $this->session->data['currency'], 0, false);
				} else {
					$tax_price = false;
				}
				...
				$data['products'][] = array(
					...
					'retail_price'=> ($this->config->get('config_customer_group_id') != 1 ? round($product_info['retail_price']) : false),
