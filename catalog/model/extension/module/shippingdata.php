...
class ModelModuleShippingData extends Model {
    ...
    public function getUkrPoshtaCities($region, $search = '') {
        ...
        $sql = "SELECT *, `CITY_" . $ukrposhta->lang . "` as `description`, CONCAT(`DISTRICT_" . $ukrposhta->lang . "`, '" . $district_text . "') as `district_description`,  CONCAT(`REGION_" . $ukrposhta->lang . "`, '" . $region_text . "') as `region_description` FROM `" . DB_PREFIX . "ukrposhta_cities` WHERE 1";
    ...
    public function getUkrPoshtaDepartments($city, $search = '') {
        require_once(DIR_SYSTEM . 'helper/ukrposhta.php');
        ...
        $sql = "SELECT *, `POSTINDEX` as `postcode`, CONCAT(`POSTINDEX`, ', ', `ADDRESS`) as `description` FROM `" . DB_PREFIX . "ukrposhta_departments` WHERE `TYPE_ACRONYM`='МВПЗ' AND (`CITY_UA` = '" . $this->db->escape($city_parts[0]) . "' OR `CITY_EN` = '" . $this->db->escape($city_parts[0]) . "' OR `CITY_RU` = '" . $this->db->escape($city_parts[0]) . "')";
    ...
    public function getShippingMethod() {
        if (!empty($this->request->post['shipping']) && is_string($this->request->post['shipping'])) {
