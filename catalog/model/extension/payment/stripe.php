<?php
//==============================================================================
// Stripe Payment Gateway Pro v2025-6-09 (also set below)
// 
...
class ModelExtensionPaymentStripe extends Model {
	...
	private $name = 'stripe';
	private $extension_version = 'v2025-6-09';
		...
		if (!empty($this->session->data['order_id'])) {
			...
			if ($customer_id) {
				...
				if (!empty($customer['address_id'])) {
					$customer['address'] = $this->db->query("SELECT * FROM " . DB_PREFIX . "address WHERE address_id = " . (int)$customer['address_id'])->row;
				} elseif (version_compare(VERSION, '4.0', '>=')) {
					$customer['address'] = $this->db->query("SELECT * FROM " . DB_PREFIX . "address WHERE customer_id = " . (int)$customer_id . " AND `default` = 1")->row;
				}
			} else {
			...
			$order_info = array(
				...
				'customer_group_id'			=> (!empty($customer['customer_group_id'])) ? $customer['customer_group_id'] : $this->config->get('config_customer_group_id'),
				...
				'payment_address_id'		=> (!empty($customer['address']['address_id'])) ? $customer['address']['address_id'] : '',
				...
				'payment_zone'				=> (!empty($zone['name'])) ? $zone['name'] : '',
				'payment_zone_code'			=> (!empty($zone['code'])) ? $zone['code'] : '',
				'payment_country_id'		=> $country_id,
				'payment_country'			=> (!empty($country['name'])) ? $country['name'] : '',
				'payment_iso_code_2'		=> (!empty($country['iso_code_2'])) ? $country['iso_code_2'] : '',
				'payment_iso_code_3'		=> (!empty($country['iso_code_3'])) ? $country['iso_code_3'] : '',
				'payment_address_format'	=> (version_compare(VERSION, '4.0', '<')) ? $country['address_format'] : '',
				'shipping_address_id'		=> (!empty($this->session->data['shipping_address']['address_id'])) ? $this->session->data['shipping_address']['address_id'] : '',
	...
	public function createOrder($order_data) {
		...
		$shipping_code = '';
		$shipping_method = (version_compare(VERSION, '4.0.2.0', '<')) ? '' : array();
		...
		if (!empty($this->session->data['shipping_method'])) {
			if (version_compare(VERSION, '4.0', '<')) {
				...
				$shipping_code = $this->session->data['shipping_methods'][$shipping[0]]['quote'][$shipping[1]]['code'];
				$shipping_method = $this->session->data['shipping_methods'][$shipping[0]]['quote'][$shipping[1]]['title'];
			} else {
				$shipping_code = (isset($this->session->data['shipping_method']['code'])) ? $this->session->data['shipping_method']['code'] : '';
		...
		$default_order_data = array(
			...
			'payment_method'			=> (version_compare(VERSION, '4.0.2.0', '<') ? html_entity_decode($settings['title_' . $language], ENT_QUOTES, 'UTF-8') : array('code' => $this->name, 'name' => html_entity_decode($settings['title_' . $language], ENT_QUOTES, 'UTF-8'))),
		...
		foreach (array('payment', 'shipping') as $address_type) {
			if (empty($data[$address_type . '_address_format']) && !empty($data[$address_type . '_country_id'])) {
				$country = $this->db->query("SELECT * FROM " . DB_PREFIX . "country WHERE country_id = " . (int)$data[$address_type . '_country_id'])->row;
				if (!empty($country['address_format'])) {
					$data[$address_type . '_address_format'] = $country['address_format'];
				} elseif (!empty($country['address_format_id'])) {
					$data[$address_type . '_address_format'] = $this->db->query("SELECT * FROM " . DB_PREFIX . "address_format WHERE address_format_id = " . (int)$country['address_format_id'])->row['address_format'];
				}
			}
		}
	...
	// metadata()
	//==============================================================================
	public function metadata($order_info) {
	...
	// replaceShortcodes()
	//==============================================================================
	public function replaceShortcodes($text, $order_info) {
		$product_names = array();
