...
class ModelExtensionPaymentBankTransfer extends Model {
	public function getMethod($address, $total) {
		...
		if ($this->config->get('payment_bank_transfer_total') > 0 && $this->config->get('payment_bank_transfer_total') > $total) {
 		...
 		if (!empty($this->config->get('payment_bank_transfer_currency')) && $this->config->get('payment_bank_transfer_currency') != 1 && $this->config->get('payment_bank_transfer_currency') != $this->session->data['currency']) {
			$status = false;
		}
