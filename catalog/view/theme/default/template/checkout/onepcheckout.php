...
class ControllerCheckoutOnepcheckout extends Controller {
	...
	public function shipping_method($render = true, &$data = array()){
		...
		// inpost address save to session
		if(isset($this->request->post['parcelLocker'])) {
			if(isset($this->session->data['shipping_method']['code']) && $this->session->data['shipping_method']['code'] == 'inpost_shipping_2.inpost_shipping_2_6') {
				$this->session->data['parcelLocker'] = $this->request->post['parcelLocker'];
				$data['parcelLocker'] = $this->request->post['parcelLocker'];

				$this->session->data['parcelAddressLocker'] = $this->request->post['parcelAddressLocker'];
				$data['parcelAddressLocker'] = html_entity_decode($this->request->post['parcelAddressLocker'], ENT_QUOTES, 'UTF-8');
			} else {
				unset($this->session->data['parcelLocker']);
				unset($this->session->data['parcelAddressLocker']);
			...
			if ($this->customer->isLogged()){
 				if(isset($client_dsc_info['dsc_shipping_method']) && $client_dsc_info['dsc_shipping_method'] == 'inpost_shipping_2.inpost_shipping_2_6') {
					if (!empty($client_dsc_info['dsc_parcelLocker']) && !empty($client_dsc_info['dsc_parcelAddressLocker'])) {
				...
				if(isset($this->session->data['shipping_method']['code']) && $this->session->data['shipping_method']['code'] == 'inpost_shipping_2.inpost_shipping_2_6') {
					if (isset($this->session->data['parcelLocker']) && isset($this->session->data['parcelAddressLocker'])) {
						$data['parcelLocker'] = $this->session->data['parcelLocker'];
						$data['parcelAddressLocker'] = html_entity_decode($this->session->data['parcelAddressLocker'], ENT_QUOTES, 'UTF-8');
					} else {
						$data['parcelLocker'] = '';
						$data['parcelAddressLocker'] = '';
					}
		...
		if (isset($this->session->data['shipping_method']['code'])) {
			...
			// inpost address save to session
			if(isset($this->session->data['parcelLocker']) && $data['shipping_code'] == 'inpost_shipping_2.inpost_shipping_2_6') {
				$data['parcelLocker'] = $this->session->data['parcelLocker'];
			}
			if(isset($this->session->data['parcelAddressLocker']) && $data['shipping_code'] == 'inpost_shipping_2.inpost_shipping_2_6') {
				$data['parcelAddressLocker'] = html_entity_decode($this->session->data['parcelAddressLocker'], ENT_QUOTES, 'UTF-8');
			}
			// inpost address save to session
	...
	public function payment_method($render = true, &$data = array()) {
		...
		// Przelewy24 Bank's
        $url = "https://secure.przelewy24.pl/api/v1/payment/methods/PL/?amount=150&currency=PLN";
